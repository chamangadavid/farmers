<!-- resources\js\Pages\MyFarmer\GeneralExpenses\EditGeneralExpenses.vue -->
<script setup>
...
const populateForm = (expense) => {
    ...
    form.expense_date = expense.expense_date ?? null
    form.expense_category = expense.expense_category ?? undefined
    form.expense_name = expense.expense_name ?? ''
    form.description = expense.description ?? ''
    form.amount = expense.amount !== null && expense.amount !== undefined ? Number(expense.amount) : null
    form.payment_method = expense.payment_method ?? undefined
...
const submit = async () => {
    try {
        ...
        await axios.put(`/general-expenses/${props.expense.id}`, payload)
        ...
        if (error.response?.status === 422) {
            ...
            message.error('Failed to update general expense.')
        }
    }
    finally {
        loading.value = false
...
<template>
    ...
    <a-modal :open="open" title="Edit General Expense" :width="900" :confirm-loading="loading" ok-text="Update Expense"
