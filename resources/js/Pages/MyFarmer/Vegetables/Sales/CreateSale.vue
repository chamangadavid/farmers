...
const loadHarvests = async () => {
    try {
        const { data } = await axios.get('/all-vegetable-sales/harvests')
        harvests.value = data
    }
    catch {
        message.error(
...
watch(() => form.vegetable_harvest_id, (id) => {
    const harvest = harvests.value.find(x => x.id === id)
    selectedHarvest.value = harvest
    if (!harvest) {
        return
    }

    availableQuantity.value = Number(harvest.quantity)
    form.unit_price = Number(harvest.unit_price)
}
)

watch([() => form.quantity, () => form.unit_price], () => {
    form.total_amount = Number(form.quantity || 0) * Number(form.unit_price || 0)
}
)

watch(() => form.quantity, (value) => {
    if (Number(value) > availableQuantity.value) {
        message.warning(
...
const saveSale = async () => {
    try {
        ...
        await axios.post('/vegetable-sales', {
            ...form,
            sale_date: dayjs(form.sale_date).format('YYYY-MM-DD')
        }
        )
        ...
        console.log(error)
        message.error('Unable to save sale.')
    }
    finally {
        loading.value = false
    }
                    ...
                    <a-form-item label="Harvest" name="vegetable_harvest_id">
                        <a-select v-model:value="form.vegetable_harvest_id" show-search placeholder="Select Harvest">
                            <a-select-option v-for="item in harvests" :key="item.id" :value="item.id">
                                {{ item.production?.vegetableType?.name }} - {{ item.production?.batch_number }} - {{
                                item.quantity }} Kg
                            </a-select-option>
                        </a-select>
                    </a-form-item>
